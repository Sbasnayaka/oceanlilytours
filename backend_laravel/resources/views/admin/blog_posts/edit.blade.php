        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="md:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="title" value="{{ $post->title }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                </div>
                
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Slug (Optional)</label>
                    <input type="text" name="slug" value="{{ $post->slug }}" placeholder="Auto-generated if left empty" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                </div>
                
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Excerpt (Short description)</label>
                    <textarea name="excerpt" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">{{ $post->excerpt }}</textarea>
                </div>
                
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Post Content <span class="text-red-500">*</span></label>
                    <textarea name="content" id="rich-editor" class="w-full border border-gray-300 rounded-lg">{{ $post->content }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    @if($post->featured_image)
                        <img src="{{ $post->featured_image }}" class="w-full h-32 object-cover rounded-lg mb-3 border border-gray-200">
                    @endif
                    ...
                    <p class="text-xs text-gray-500 mt-2">Upload a new image to replace the existing one.</p>
                </div>
                
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="published" value="1" {{ $post->published ? 'checked' : '' }} class="w-5 h-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
                        <span class="ml-2 text-sm font-bold text-gray-700">Published</span>
                    </label>
                    
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="featured" value="1" {{ $post->featured ? 'checked' : '' }} class="w-5 h-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
