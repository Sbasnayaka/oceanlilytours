...
<div class="mb-6 flex items-center gap-4">
    ...
    <h2 class="text-2xl font-bold text-gray-800">Edit Menu Item</h2>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8 max-w-2xl">
    <form action="{{ route('navbar-items.update', $item->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 mb-8">
            <div>
                ...
                <input type="text" name="label" value="{{ old('label', $item->label) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">URL / Path</label>
                <input type="text" name="url" value="{{ old('url', $item->url) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
            </div>
            ...
            <div>
                ...
                <select name="parent_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
                    ...
                    @foreach($parents as $parent)
                        <option value="{{ $parent->id }}" {{ old('parent_id', $item->parent_id) == $parent->id ? 'selected' : '' }}>{{ $parent->label }}</option>
                    @endforeach
                </select>
            </div>
