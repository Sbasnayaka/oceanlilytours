...
@section('title', 'Add New Tour Package Builder')

@section('content')
<div class="mb-4 flex justify-between items-center">
    <h2 class="text-2xl font-bold text-gray-800">Add New Tour</h2>
...
<form action="{{ route('packages.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Sub Heading (Top Banner Text)</label>
                <input type="text" name="sub_heading" value="{{ old('sub_heading') }}" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            ...
            <div class="pt-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50 p-4 rounded border border-gray-200">
                    ...
                    @empty
                        <span class="text-sm text-red-500 col-span-4">No categories created yet! Please create categories first.</span>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="number" step="0.01" name="price" value="{{ old('price') }}" required placeholder="e.g. 1200" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Duration (Days) *</label>
                <input type="number" name="duration_days" value="{{ old('duration_days', 1) }}" required placeholder="e.g. 14" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Tour Type</label>
                <input type="text" name="tour_type" value="{{ old('tour_type') }}" placeholder="e.g. Wellness" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Min / Max People *</label>
                <input type="number" name="max_persons" value="{{ old('max_persons', 10) }}" required placeholder="e.g. 2" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Location Count</label>
                <input type="text" name="location_count" value="{{ old('location_count') }}" placeholder="e.g. 8 Destinations" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            <div class="col-span-1 md:col-span-3 flex items-center pt-5">
                <label class="flex items-center space-x-2 text-sm cursor-pointer hover:text-green-600 transition">
                    <input type="checkbox" name="featured" value="1" class="rounded text-green-600 focus:ring-green-500" {{ old('featured') ? 'checked' : '' }}>
                    <span class="font-bold text-green-600">Mark as Featured Base Package</span>
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Main Background Image</label>
                ...
                <p class="text-[10px] text-gray-400 mt-1 uppercase">Recommended Size: 1920x1080px</p>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">Map Embed Code (Iframe)</label>
                <textarea name="map_embed_code" rows="3" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm font-mono text-xs" placeholder='<iframe src="..."></iframe>'>{{ old('map_embed_code') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="journey_highlights" class="rich-editor">{{ old('journey_highlights') }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-semibold text-red-500 mb-1">Insightful Tips (Pink Box)</label>
                <textarea name="insightful_tips" class="rich-editor">{{ old('insightful_tips') }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">FAQ Content</label>
                <textarea name="faq_content" class="rich-editor">{{ old('faq_content') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl shadow-sm border border-blue-100 p-6">
        ...
        <div id="itinerary-container" class="space-y-4">
            <!-- Dynamic days will appear here via JavaScript -->
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="seo_title" value="{{ old('seo_title') }}" placeholder="Custom title for search engines (leave empty to use tour name)" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">SEO Meta Description</label>
                <textarea name="seo_description" rows="2" placeholder="Brief summary for search results..." class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">{{ old('seo_description') }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1">SEO Keywords</label>
                <input type="text" name="seo_keywords" value="{{ old('seo_keywords') }}" placeholder="hiking, wellness, sri lanka, etc." class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded focus:bg-white focus:ring-2 focus:ring-blue-600 outline-none transition text-sm">
            </div>
        </div>
    </div>

    <!-- Hidden required system fields that UI doesn't need to bloat -->
    ...
    <button type="submit" class="w-full py-4 bg-cyan-500 hover:bg-cyan-600 text-white font-bold text-lg rounded-lg shadow-lg transition transform hover:-translate-y-0.5">
        <i class="fas fa-save mr-2"></i> Save Tour Details
    </button>
</form>
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function addDayBlock() {
            ...
            // We generate the DOM exactly using array bracket notation: itineraries[INDEX][field]
            // We use Date.now() as a unique index to prevent array clashing if deleted out of order
                ...
                <div class="bg-white border text-sm border-blue-200 rounded-lg p-4 shadow-sm relative itinerary-block transition-all" id="itin-block-${idx}">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <!-- NOTE: We use textarea. It can be turned into TinyMCE, but for safety in dynamic insertions, simple textarea is heavily preferred to prevent JS init crashes. -->
