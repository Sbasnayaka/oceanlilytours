...
<div class="mb-6 flex items-center gap-4">
    ...
    <h2 class="text-2xl font-bold text-gray-800">Add Guest Testimonial</h2>
</div>
    ...
    <form action="{{ route('testimonials.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="text" name="location" value="{{ old('location') }}" placeholder="e.g., London, UK" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Optional. Max 2MB. JPG, PNG, WEBP.</p>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-bold text-gray-700 mb-1">Review Text <span class="text-red-500">*</span></label>
                <textarea name="review_text" rows="4" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition">{{ old('review_text') }}</textarea>
            </div>

            <div class="flex gap-6">
                <label class="flex items-center cursor-pointer gap-2">
                    <input type="checkbox" name="featured" value="1" {{ old('featured') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded border-gray-300">
                    <span class="text-sm font-bold text-gray-700">Featured on Homepage</span>
                </label>
                <label class="flex items-center cursor-pointer gap-2">
                    <input type="checkbox" name="verified" value="1" checked class="w-4 h-4 text-blue-600 rounded border-gray-300">
